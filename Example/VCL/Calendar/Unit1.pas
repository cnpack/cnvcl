...
begin
  ...
  mmoResult.Lines.Add('农历二十八宿：'+ Get28XiuFromNumber(GetLunar28XiuFromDay(AYear, AMonth, ADay)) + '/' + Get28XiuLongFromNumber(GetLunar28XiuFromDay(AYear, AMonth, ADay)));
...
begin
  ...
  for I := Low(SCnLeapNumber) to High(SCnLeapNumber) - 1 do
  begin
    if not ((SCnLeapNumber[I + 1] - SCnLeapNumber[I]) in [0, 1]) then
      mmoDays.Lines.Add(IntToStr(I) + ' ' + IntToStr(SCnLeapNumber[I]));
  end;
  ...
  begin
    ...
    begin
      if Sum = SCnLeapNumber[I - 1] then
        Equ := ''
      else
        Equ := ' !';

      if SCnLeapMonth[I] <> '0' then
        mmoDays.Lines.Add(Format('Year: %d ... +%d %d %s %s', [I - 1 - 849, Sum, SCnLeapNumber[I - 1], Equ, SCnLeapMonth[I]]))
      else
        mmoDays.Lines.Add(Format('Year: %d ... +%d %d %s', [I - 1 - 849, Sum, SCnLeapNumber[I - 1], Equ]));
    end
    else
    begin
      if Sum = SCnLeapNumber[I - 2] then
        Equ := ''
      else
        Equ := ' !';

      if SCnLeapMonth[I] <> '0' then
        mmoDays.Lines.Add(Format('Year: %d ... +%d %d %s %s', [I - 1 - 849, Sum, SCnLeapNumber[I - 2], Equ, SCnLeapMonth[I]]))
      else
        mmoDays.Lines.Add(Format('Year: %d ... +%d %d %s', [I - 1 - 849, Sum, SCnLeapNumber[I - 2], Equ]));
    end;
  end;
end;
