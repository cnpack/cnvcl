...
* 修改记录：2026.02.13 V0.13
*               增加 TCnCOlorSpaceConverter，暂未完整测试
*           2022.09.25 V0.12
*               增加 Win64 位的支持
...
var
  ...
  GdiActTimer: TTimer;        // GDI 资源释放定时器
  DefGdiAllocStyle: TGdiAllocStyle = gsNormal; // 默认 GDI 释放方式
  FreeGdiWaitTime: Cardinal = 3000; // 自动释放 GDI 资源等待时间
...
begin
  ...
  try
{$IFDEF FPC}
    Bmp.LoadFromClipboardFormat(AFormat);
{$ELSE}
...
begin
  ...
  try
    AssignTo(Bmp);
{$IFDEF FPC}
    Bmp.SaveToClipboardFormat(Format);
{$ELSE}
...
begin
  ...
  begin
    ...
    begin
      ...
      Delta := RowInc - tx * 3;
{$IFDEF FPC}
      Pix := PCnColor(TCnNativeInt(FScanLine[y]) + x * 3);
{$ELSE}
...
{$IFDEF FPC}
        Pix := PCnColor(TCnNativeInt(Pix) + Delta);
{$ELSE}
