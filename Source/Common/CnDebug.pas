...
begin
  case AMessage of  // Windows Messages
    ...
    CM_BASE + 68            : Result := Format('CM_ISSHORTCUT: %d/$%x', [CM_ISSHORTCUT, CM_ISSHORTCUT]);
    CM_BASE + 69            : Result := Format('CM_UPDATEACTIONS: %d/$%x', [CM_UPDATEACTIONS, CM_UPDATEACTIONS]);
    CM_BASE + 70            : Result := Format('CM_INVALIDATEDOCKHOST: %d/$%x', [CM_INVALIDATEDOCKHOST, CM_INVALIDATEDOCKHOST]);
    CM_BASE + 71            : Result := Format('CM_SETACTIVECONTROL: %d/$%x', [CM_SETACTIVECONTROL, CM_SETACTIVECONTROL]);
    CM_BASE + 72            : Result := Format('CM_POPUPHWNDDESTROY: %d/$%x', [CM_POPUPHWNDDESTROY, CM_POPUPHWNDDESTROY]);
    CM_BASE + 73            : Result := Format('CM_CREATEPOPUP: %d/$%x', [CM_CREATEPOPUP, CM_CREATEPOPUP]);
    CM_BASE + 74            : Result := Format('CM_DESTROYHANDLE: %d/$%x', [CM_DESTROYHANDLE, CM_DESTROYHANDLE]);
    CM_BASE + 75            : Result := Format('CM_MOUSEACTIVATE: %d/$%x', [CM_MOUSEACTIVATE, CM_MOUSEACTIVATE]);
    CM_BASE + 76            : Result := Format('CM_CONTROLLISTCHANGING: %d/$%x', [CM_CONTROLLISTCHANGING, CM_CONTROLLISTCHANGING]);
    CM_BASE + 77            : Result := Format('CM_BUFFEREDPRINTCLIENT: %d/$%x', [CM_BUFFEREDPRINTCLIENT, CM_BUFFEREDPRINTCLIENT]);
    CM_BASE + 78            : Result := Format('CM_UNTHEMECONTROL: %d/$%x', [CM_UNTHEMECONTROL, CM_UNTHEMECONTROL]);
    CM_BASE + 79            : Result := Format('CM_DOUBLEBUFFEREDCHANGED: %d/$%x', [CM_DOUBLEBUFFEREDCHANGED, CM_DOUBLEBUFFEREDCHANGED]);
    CM_BASE + 80            : Result := Format('CM_PARENTDOUBLEBUFFEREDCHANGED: %d/$%x', [CM_PARENTDOUBLEBUFFEREDCHANGED, CM_PARENTDOUBLEBUFFEREDCHANGED]);
    CM_BASE + 81            : Result := Format('CM_STYLECHANGED: %d/$%x', [CM_STYLECHANGED, CM_STYLECHANGED]);
    CM_BASE + 82            : Result := Format('CM_GESTURE: %d/$%x', [CM_GESTURE, CM_GESTURE]);
    CM_BASE + 83            : Result := Format('CM_CUSTOMGESTURESCHANGED: %d/$%x', [CM_CUSTOMGESTURESCHANGED, CM_CUSTOMGESTURESCHANGED]);
    CM_BASE + 84            : Result := Format('CM_GESTUREMANAGERCHANGED: %d/$%x', [CM_GESTUREMANAGERCHANGED, CM_GESTUREMANAGERCHANGED]);
    CM_BASE + 85            : Result := Format('CM_STANDARDGESTURESCHANGED: %d/$%x', [CM_STANDARDGESTURESCHANGED, CM_STANDARDGESTURESCHANGED]);
    CM_BASE + 86            : Result := Format('CM_INPUTLANGCHANGE: %d/$%x', [CM_INPUTLANGCHANGE, CM_INPUTLANGCHANGE]);
    CM_BASE + 87            : Result := Format('CM_TABLETOPTIONSCHANGED: %d/$%x', [CM_TABLETOPTIONSCHANGED, CM_TABLETOPTIONSCHANGED]);
    CM_BASE + 88            : Result := Format('CM_PARENTTABLETOPTIONSCHANGED: %d/$%x', [CM_PARENTTABLETOPTIONSCHANGED, CM_PARENTTABLETOPTIONSCHANGED]);
    CM_BASE + 89            : Result := Format('CM_CUSTOMSTYLECHANGED: %d/$%x', [CM_CUSTOMSTYLECHANGED, CM_CUSTOMSTYLECHANGED]);
    CM_BASE + 90            : Result := Format('CM_SYSFONTSALLCHANGED: %d/$%x', [CM_SYSFONTSALLCHANGED, CM_SYSFONTSALLCHANGED]);
    CM_BASE + 91            : Result := Format('CM_PARENTVISIBLECHANGED: %d/$%x',  [CM_PARENTVISIBLECHANGED, CM_PARENTVISIBLECHANGED]);
    CM_BASE + 92            : Result := Format('CM_SYSCOMMAND: %d/$%x', [CM_SYSCOMMAND, CM_SYSCOMMAND]);
    CM_BASE + 93            : Result := Format('CM_REMOTESESSIONSTATUSCHANGED: %d/$%x', [CM_REMOTESESSIONSTATUSCHANGED, CM_REMOTESESSIONSTATUSCHANGED]);
    CM_BASE + 94            : Result := Format('CM_STYLEELEMENTSCHANGED: %d/$%x', [CM_STYLEELEMENTSCHANGED, CM_STYLEELEMENTSCHANGED]);
