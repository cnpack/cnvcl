...
================================================================================
* 软件名称：开发包基础库
* 单元名称：多杂凑函数映射的快速查找 BloomFilter 实现单元
* 单元作者：CnPack 开发组
* 备    注：杂凑函数数量 = ln2 * (Bit 长度 / 字符串容量) 时判断最优，但数据量大时
*               杂凑函数数量不宜过大，以免计算耗时。所以分六个档次：
*               1、杂凑函数数量 4，比率 32，用于小规模数据，计算量小，单位内存大
*               2、杂凑函数数量 6，比率 30
*               3、杂凑函数数量 8，比率 25
*               4、杂凑函数数量 10，比率 20
*               5、杂凑函数数量 12，比率 18
*               6、杂凑函数数量 15，比率 15，用于大规模数据，计算量大，单位内存小
*               多杂凑函数采用 CRC32 及不同的初始值来做
