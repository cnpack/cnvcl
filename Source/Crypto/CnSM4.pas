...
type
  ...
  TSM4Context = packed record
    ...
    Sk: array[0..SM4_KEYSIZE * 2 - 1] of Cardinal;  {!<  SM4 subkeys     }
  end;
...
procedure SM4EncryptStreamCBC(Source: TStream; Count: Cardinal;
  const Key: TSM4Key; const InitVector: TSM4Iv; Dest: TStream);
...
const
  ...
  CK: array[0..SM4_KEYSIZE * 2 - 1] of Cardinal = (
...
begin
  ...
  Result := PCnByte(TCnNativeInt(PTable) + Inch)^;
end;
    ...
    begin
      ...
      begin
        for I := 0 to SM4_BLOCKSIZE - 1 do
          (PCnByte(TCnNativeInt(Output) + I))^ := (PCnByte(TCnNativeInt(Input) + I))^
            xor LocalIv[I];
      ...
      begin
        ...
        for I := 0 to SM4_BLOCKSIZE - 1 do
          (PCnByte(TCnNativeInt(Output) + I))^ := EndBuf[I]
            xor LocalIv[I];
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for I := 0 to SM4_BLOCKSIZE - 1 do
          (PCnByte(TCnNativeInt(Output) + I))^ := (PCnByte(TCnNativeInt(Output) + I))^
            xor LocalIv[I];
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for I := 0 to SM4_BLOCKSIZE - 1 do
          (PCnByte(TCnNativeInt(Output) + I))^ := (PCnByte(TCnNativeInt(Input) + I))^
            xor (PCnByte(TCnNativeInt(Output) + I))^;  // 加密结果与明文异或作为输出密文
      ...
      begin
        ...
        for I := 0 to Length - 1 do // 只需异或剩余长度，无需处理完整的 16 字节
          (PCnByte(TCnNativeInt(Output) + I))^ := (PCnByte(TCnNativeInt(Input) + I))^
            xor (PCnByte(TCnNativeInt(Output) + I))^;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for I := 0 to SM4_BLOCKSIZE - 1 do
          (PCnByte(TCnNativeInt(Output) + I))^ := (PCnByte(TCnNativeInt(Output) + I))^
            xor (PCnByte(TCnNativeInt(Input) + I))^;    // 加密结果与密文异或得到明文
      ...
      begin
        ...
        for I := 0 to Length - 1 do
          (PCnByte(TCnNativeInt(Output) + I))^ := (PCnByte(TCnNativeInt(Output) + I))^
            xor (PCnByte(TCnNativeInt(Input) + I))^;
      end;
...
procedure SM4CryptCtr(var Ctx: TSM4Context; Mode: Integer; Length: Integer;
...
begin
  ...
  Len := Length(Nonce);
  if Len < SM4_NONCESIZE then // Nonce 长度小于 16 字节补 0
  begin
    SetLength(Nonce, SM4_NONCESIZE);
    for I := Len to SM4_NONCESIZE - 1 do
      Nonce[I] := 0;
  end;
...
procedure SM4EncryptStreamECB(Source: TStream; Count: Cardinal;
  const Key: TSM4Key; Dest: TStream);
var
