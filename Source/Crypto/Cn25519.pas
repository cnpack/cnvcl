...
* 单元作者：刘啸
* 备    注：本单元目前实现了 RFC 7748 中 Montgomery 椭圆曲线 y^2 = x^3 + A*X^2 + x （文中叫 Curve，参数 A）
*           以及扭曲 Edwards 椭圆曲线 au^2 + v^2 = 1 + d * u^2 * v^2 （文中叫 Ed，参数 a d）的点加减乘运算算法及签名验签算法。
*
*           运算算法中已实现仅基于 X 以及蒙哥马利阶梯的快速标量乘、扩展四元坐标的快速点加、
*           以及结合多项式约减代替模运算所进行的加速算法，是原始点加算法速度的五十倍以上。
*
*           签名验签算法则基于 RFC 8032 中说明的规则。
*
...
*               一对公私钥。
*
*           本单元中的公私钥既可按常规的 ECC 公私钥处理，也可用 LoadFromData/SaveToData 方法与 32/57 字节内容加载存储。
*
...
*               但 Ed448 曲线是非扭曲爱德华曲线导致不能与 Ed25519 一样加速，只能用三元射影点加算法加速，均较慢。
*
