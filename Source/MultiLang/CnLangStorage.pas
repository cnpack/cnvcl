...
type
  ...
  protected
    ...
    function GetAuthor(var Value: string): Boolean;

    function GetAuthorEmail(var Value: string): Boolean;
  ...
  private
    ...
    FFileName: string;
    FDesignLangPath: string;
    FDesignLangFile: string;
    procedure SetLanguagePath(Value: string);
  ...
  protected
    ...
    procedure InitFromAFile(const AFileName: string); virtual;

  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure SetDesignLangPath(const aPath: string);
    procedure SetDesignLangFile(const aFile: string);

    function GetCurrentLanguageFileName: string; virtual;
   {* 获得当前语言的语言文件名，包括扩展名 }
    class function GetLanguageFileExt: string; virtual;
    {* 抽象方法，获得所有语言文件的统一扩展名 }
    function IsLanguageFile(const FileName: string): Boolean; virtual; abstract;
    ...
    property DesignLangPath: string read FDesignLangPath;
    property DesignLangFile: string read FDesignLangFile;
  end;
...
function TCnCustomLangStorage.GetAuthor(var Value: string): Boolean;
begin
  Result := GetString(SystemNamePrefix + SCnAuthor, Value);
end;

function TCnCustomLangStorage.GetAuthorEmail(var Value: string): Boolean;
begin
...
function TCnCustomLangFileStorage.GetCurrentLanguageFileName: string;
begin
...
class function TCnCustomLangFileStorage.GetLanguageFileExt: string;
begin
  Result := '.txt';
end;

procedure TCnCustomLangFileStorage.InitFromAFile(const AFileName: string);
var
  AStr: string;
  AID: LongWord;
begin
...
var
  Sr: TSearchRec;
  ActualPath,  AFileName, ASearchPath: string;
begin
...
procedure TCnCustomLangFileStorage.SetFileName(const Value: string);
begin
  FFileName := Value;
...
procedure TCnCustomLangFileStorage.SetLanguagePath(Value: string);
begin
...
procedure TCnCustomLangFileStorage.SetDesignLangPath(const aPath: string);
begin
  if csDesigning in ComponentState then
    FDesignLangPath := aPath;
end;

procedure TCnCustomLangFileStorage.SetDesignLangFile(const aFile: string);
begin
  if csDesigning in ComponentState then
