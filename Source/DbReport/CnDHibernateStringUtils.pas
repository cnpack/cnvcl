...
* 单元标识：$Id: CnDHibernateStringUtils.pas,v 1.3 2009/05/19 11:45:25 liuxiao Exp $
...
begin
  ...
  if Length(Result) > 0 then
    CharToOemBuff({$IFDEF UNICODE} PWideChar{$ELSE}PAnsiChar{$ENDIF}(AnsiStr), PAnsiChar(Result), Length(Result));
end;
...
begin
  ...
  if Length(Result) > 0 then
    OemToCharBuff(PAnsiChar(OemStr), {$IFDEF UNICODE} PWideChar{$ELSE}PAnsiChar{$ENDIF}(Result), Length(Result));
end;
