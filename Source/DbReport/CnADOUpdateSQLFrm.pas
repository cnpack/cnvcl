...
* 单元标识：$Id: CnADOUpdateSQLFrm.pas,v 1.3 2009/01/02 08:27:38 liuxiao Exp $
* 修改记录：2008.04.25
*               创建单元
================================================================================
|</PRE>}
...
begin
  ...
  begin
    ...
    begin
      //从ModifySQL字符串中获取表名
      FTableName := Trim(FModifySQL.Strings[0]);
      System.Delete(FTableName, 1, 7);
      FTableName := Trim(FTableName);
      cbbTables.ItemIndex := cbbTables.Items.IndexOf(FTableName);

      //根据FTableName表名获取字段名
      FConnection.GetFieldNames(FTableName, lstKeyFields.Items);
...
begin
  ...
  begin
    ...
    begin
      sFieldName := lstUpdateFields.Items.Strings[i];
      if (lstUpdateFields.SelCount - 1) <> j then //不是选中项的最后一项时
      begin
        FModifySQL.Add('    ' + sFieldName + ' = :' + sFieldName + ',');
  ...
  FInsertSQL.Add(sFieldNames + ')' + #13 + #10 + sValuesParams + ')');
  ...
  begin
    ...
    begin
      sFieldName := lstKeyFields.Items.Strings[i];
      if (lstKeyFields.SelCount - 1) <> j then //不是选中项的最后一项时
      begin
        FModifySQL.Add('    ' + sFieldName + ' = :OLD_' + sFieldName + ' and ');
        FDeleteSQL.Add('    ' + sFieldName + ' = :OLD_' + sFieldName + ' and ');
      end
      else begin                              //是选中项的最后一项时
        FModifySQL.Add('    ' + sFieldName + ' = :OLD_' + sFieldName);
        FDeleteSQL.Add('    ' + sFieldName + ' = :OLD_' + sFieldName);
      end;
      Inc(j);
    end;
  end;
