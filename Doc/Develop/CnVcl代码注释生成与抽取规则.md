...
interface 部分每个独立的、以 `TCn` 开头的类型声明，只要不是简单的前向声明，其“名字所在行”的下一行必须写一行独立注释，格式固定为：
...
所有对外 `function/procedure/constructor/destructor`以及`property` 均必须在声明后添加文档注释块，并按固定结构编写：
...
注意“参数”和“返回值”后均有个全角冒号，功能说明的末尾需要全角句号结尾。没有返回值的写“返回值：（无）”
