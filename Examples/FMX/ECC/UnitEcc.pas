...
uses
  ...
  Chart,} TypInfo, CnPrimeNumber, CnBigNumber, CnNativeDecl, CnCommon, CnPemUtils, CnPolynomial, FMX.ComboEdit, FMX.Edit, FMX.Memo, FMX.TabControl, FMX.Types,
  FMX.ScrollBox, FMX.Controls.Presentation;

type
  TFormEcc = class(TForm)
    pgc1: TTabControl;
    tsSimpleECC: TTabItem;
    ...
    cbbKeyHash: TComboEdit;
    btnKeyGenerate: TButton;
    btnKeyLoadSig: TButton;
    cbbCurveTypes: TComboEdit;
...
begin
  ...
  FEcc64E2311.PointAddPoint(P, Q, P);
  lblAddResult.Text := IntToStr(P.X) + ',' + IntToStr(P.Y);
end;
...
begin
  ...
  lblMResult.Text := IntToStr(P.X) + ',' + IntToStr(P.Y);
end;
...
begin
  ...
  begin
    ...
    begin
      lblCurveTypeText.Text := GetEnumName(TypeInfo(TCnEccCurveType), Ord(FCurveType));
...
begin
  ...
  begin
    ...
    lblCurveTypeText.Text := GetEnumName(TypeInfo(TCnEccCurveType), cbbCurveTypes.ItemIndex);
